// Credit to https://github.com/mlarocca/AlgorithmsAndDataStructuresInAction/blob/master/JavaScript/src/common/hash.js

const MURMUR_C1 = 0xcc9e2d51
const MURMUR_C2 = 0x1b873593

// 32 bit FNV_Prime = 2**24 + 2**8 + 0x93
const FNV1_PRIME_MUL = (hash: number) =>
  (hash << 24) + (hash << 8) + (hash << 7) + (hash << 4) + (hash << 1)
const FNV1_OFFSET_BASIS = 0x811c9dc5

const ERROR_MSG_HASH_KEY_TYPE = (fname: string, val: any) =>
  `Illegal parameter for ${fname}: key = ${val} must be a String`
const ERROR_MSG_HASH_KEY_EMPTY = (fname: string) =>
  `Illegal parameter for ${fname}: key must be a non empty string`
const ERROR_MSG_HASH_SEED = (fname: string, val: any) =>
 ...
 * @returns {number}
 */
export const fnv1Hash32 = (key: string) => {
  if (key.length === 0) {
